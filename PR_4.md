# Solutions - Application Structure Definition (v4)
...
## Data Entry Component: Nextcell

All data entry modules (Sales Data, Market Insights, Patients News/Dropouts) use the **Nextcell** component, an Excel-like data grid that provides:

- Keyboard navigation (arrow keys, Tab, Enter)
- Multi-cell selection (click, Shift+click, Ctrl+click, drag)
...
- Calculated columns support (read-only, auto-updating)
- Hierarchical columns and rows support
- Conditional styling based on data values

For detailed technical specifications of the Nextcell component, see `pr_Nextcell.md`.
...
- **Excel Load functionality**: Available via "Excel Load" button (top-left of data grid)
- Data matrix structure with **grouped rows** and **nested columns**:
  - **Column structure**:
    ...
    - **Subcolumns** (within each month):
      - Patients (editable)
      - Dose (editable)
      - Vol. (Volume) - **Calculated, read-only**
  - **Row structure**:
...
- **Vol. (Volume)**: Calculated automatically as `Patients × Dose`
  - Formula: `Vol. = Patients × Dose` (only calculates if both Patients and Dose have values)
  - Read-only (gray, non-editable)
  - Auto-updates when Patients or Dose changes
  - Excluded from Excel Load random data generation (calculated columns are automatically skipped)

**Editing behavior**:

- Click directly on cell → inline text field for editing
- Calculated fields (Vol.): Read-only (gray, non-editable)
- Groups can be expanded/collapsed to show/hide products
- **Excel Load**: Import data from Excel templates or generate demo data (20 rows, 75% of non-calculated cells filled with random values)
...
- **Context:** This application is a **mockup for designing a subsequent fullstack application** using T3 stack on Azure SQL. It does not have a backend and is not functional. It is solely a means to analyze, design, and share with users.
